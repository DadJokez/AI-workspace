...
3. **Send:** she gets `https://…/apps/spend-dashboard` and drops it in a Teams channel. A colleague clicks it, passes SSO, and sees either the timestamped snapshot or — if Priya chose live-via-viewer and the colleague has attested Databricks access — current data under the colleague's own entitlements. No harness, no thread, no Comparative UI beyond the page itself.
...
- **Internal subdomain, yes; public custom domains, no.** The org's need is "droppable into a Teams channel or intranet page," which an SSO-gated internal hostname fully serves. Recommend promoting apps from the `/apps/{slug}` path to a dedicated internal hostname (e.g. `apps.comparative.example/{slug}`) when #133 lands — Route 53 + ACM + ALB are already in place, so this is DNS + cert work, not platform work. A distinct hostname also gives apps a cleaner CSP/cookie boundary than living under the shell's origin.
