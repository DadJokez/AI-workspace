...
**Track A — Security & Auditability to 5/5** (the explicit target: the areas an enterprise security team scores). Authz correctness + tests (#445, #444), zero unframed injection channels (#454), zero high/critical CVEs behind a CI gate (#446, #459), key separation (#455), governed egress (#439), runtime policy *enforcement* (#410 shipped; #436 presets remain), **audit completeness by construction regardless of lane** (#456, blocked by #442), a **tamper-evident audit log** (#457), privileged-access logging (#458), a documented data lifecycle (#460), and the readiness packet — threat model + incident runbook + data-flow sheet (#461).
...
- **Connector governance** → deterministic tri-state policy, durable approvals, connection-lifecycle audit, and `/admin/connectors` (#410; shipped in #831-#835). Named autonomy presets remain #436.
