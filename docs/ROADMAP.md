...
**What's live:** GitHub MCP is working end-to-end — users connect via OAuth, tokens are stored encrypted in `oauth_tokens`, the GitHub server is represented in `mcp_servers`, and the Cursor runtime mounts it per-user with a short-lived Bearer token on each turn. Tool calls/results persist on chat messages and render as compact, expandable agent activity inside chat, MCP tool executions write audit rows that admins can inspect at `/admin/audit`, long turns tolerate browser disconnects cleanly, and the manual Developer Briefing route persists GitHub workflow runs in `recipe_runs`.
