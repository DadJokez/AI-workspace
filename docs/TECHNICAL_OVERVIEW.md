...
- The registry (`packages/agent/src/models.ts`) defines the Claude tiers plus
  the registered-but-disabled candidates from #797 (Nova Pro, the four OpenAI
  GPT entries and the 2026-09-06 gaggle; scorecards in `docs/models/` and the
  runbook's honest accounting). Definition is not proof of Bedrock account
  access or production enablement — only Haiku 4.5, Sonnet 4.6 and Opus 4.7
  have seeded enablement rows.
