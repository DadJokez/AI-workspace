...
| Organization standing instructions | Internal (read by every signed-in user) | Postgres `org_instructions` | DB TLS; AWS API TLS | RDS storage encryption disabled | Admin can edit/archive; rows outlive their author (`authored_by` SET NULL) | Bedrock/AgentCore on every turn; every signed-in browser |
