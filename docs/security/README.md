...
| Authorization | Owner-scoped queries, app/skill share checks, deterministic tri-state catalog policy with durable approvals, signed write context for supported providers, and audited cross-owner admin reads | Admin is a coarse role; database-level append-only audit enforcement is pending [#457](https://github.com/DadJokez/AI-workspace/issues/457), and the unknown/external write-tool boundary remains [#701](https://github.com/DadJokez/AI-workspace/issues/701) |
