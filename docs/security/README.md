...
| Connected providers | Per-user OAuth to GitHub, Google, Notion, and Salesforce; tokens AES-256-GCM encrypted; owner/admin disconnect scrubs local credentials; connection and attestation lifecycle is audited | Disconnect does not yet call provider-side revocation APIs, and automated credential rotation/deprovisioning remain pending ([#692](https://github.com/DadJokez/AI-workspace/issues/692), [#836](https://github.com/DadJokez/AI-workspace/issues/836)) |
...
| Authorization | Owner-scoped queries, app/skill share checks, deterministic tri-state catalog policy with durable approvals, signed write context for supported providers, and audited cross-owner admin reads | Admin is a coarse role; database-level append-only audit enforcement is pending [#457](https://github.com/DadJokez/AI-workspace/issues/457), and an undeclared or externally mounted tool fails closed to needs-approval ([#701](https://github.com/DadJokez/AI-workspace/issues/701)) |
