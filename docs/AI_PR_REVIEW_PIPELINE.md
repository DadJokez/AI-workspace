...
Wired by the `feat/ai-pr-review-pipeline` PR (OIDC auth, no secrets):

- `CLAUDE.md` (review rubric) and `AGENTS.md` (Codex instructions).
- `.github/workflows/claude-code-review.yml` — auto-review, triggered by
  `CI` **and** `Product Smoke` succeeding for the same commit (the review runs
  once both are green, dedup-guarded).
- `.github/workflows/claude.yml` — `@claude` on-demand.
- The `needs-codex` label.

Auth is **Amazon Bedrock via GitHub OIDC** — no Anthropic API key and no GitHub
App private key. The workflows read two repo *variables* (not secrets):
`AWS_GHA_BEDROCK_ROLE_ARN` and `BEDROCK_REVIEW_MODEL`.

Remaining human-owned steps (Rob):

1. **AWS — GitHub OIDC role for Bedrock.** In account `351478076796`, ensure the
   OIDC provider `token.actions.githubusercontent.com` exists, then create an IAM
   role GitHub can assume and set its ARN as the repo variable
   `AWS_GHA_BEDROCK_ROLE_ARN`.
   - Trust: `sts:AssumeRoleWithWebIdentity` from
     `token.actions.githubusercontent.com`, audience `sts.amazonaws.com`, subject
     `repo:DadJokez/AI-workspace:*`.
   - Permissions: `bedrock:InvokeModel` and
     `bedrock:InvokeModelWithResponseStream` on the Claude inference profiles used.
2. **Repo variable `BEDROCK_REVIEW_MODEL`** — the Bedrock inference-profile id for
   review (e.g. the cross-region Sonnet profile).
3. **Codex Cloud** — connect it to `DadJokez/AI-workspace` so Codex can open PRs.
4. **Test PR** — open a throwaway PR and confirm: `CI` + `Product Smoke` go green,
   Claude posts a review comment, and `needs-codex` can be applied.

Quicker alternative if you'd rather skip the IAM role: replace `use_bedrock` in
both workflows with a `claude_code_oauth_token: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}`
generated by `claude setup-token` (uses your Claude subscription; still one
secret).
