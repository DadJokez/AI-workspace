...
export interface DeprovisionUserToolAccessInput {
  db?: Database;
  userId: string;
  actorUserId: string;
  reason: string;
...
/**
 * Identity-lifecycle seam for a future Ping/SCIM deprovision signal. It does
 * not disable sign-in by itself; the IdP remains authoritative for identity.
 */
export async function deprovisionUserToolAccess({
  db = getDb(),
  userId,
  actorUserId,
  reason,
  source,
  now = new Date(),
}: DeprovisionUserToolAccessInput) {
  ...
  for (const connection of activeConnections) {
    ...
    if (result.revoked) revokedConnections += 1;
    revokedAttestations += result.attestations;
  }

  await db.insert(auditLog).values({
    actorUserId,
    actionType: "user.deprovisioned",
    status: "succeeded",
    provider: "identity",
    toolName: "tool_access",
    input: { userId },
    metadata: {
  ...
  return { revokedConnections, revokedAttestations };
}
