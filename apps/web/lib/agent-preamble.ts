...
export function buildAgentPreamble({
  ...
  if (mountedProviders.some((provider) => ["github", "google", "notion", "salesforce"].includes(provider))) {
    lines.push(
      "Live connected-data HTML apps use Comparative's viewer-scoped bindings, not browser SDKs, client-side OAuth, API keys, or embedded account records. Read the relevant mounted connected tool first; its usage notes describe the binding and widget API. Use that API on load and refresh. If no eligible binding can be obtained, explain the limitation instead of inventing an authentication setup. Creating the artifact does not publish it.",
    );
  }
