 ...
 * normalized recipient email BEFORE next-auth sees them; every other auth
...
describe("[...nextauth] POST wrapper — magic-link rate limit", () => {
  it("keys the bucket on normalized email without trusting x-forwarded-for", async () => {
    ...
    expect(checkRateLimit.mock.calls[0]![1]).toBe("magic-link:tester@example.com");
  ...
  it("uses the same recipient bucket when an attacker rotates forwarding headers", async () => {
    checkRateLimit
    ...
    await POST(
      signinEmailRequest("tester@example.com", {
        "x-forwarded-for": "203.0.113.250, 10.0.0.2",
      }),
      ctx,
    );
